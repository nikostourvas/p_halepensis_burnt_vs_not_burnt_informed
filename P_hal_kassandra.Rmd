...
x <- list()
for(i in 1:length(obj_list))
x[[i]] <- stats_poppr[[i]][-nrow(stats_poppr[[1]]), variable]

y <- as.matrix(as.data.frame(x))
colnames(y) <- popNames(obj)
}
#######################3
...
ne_h_means <- colMeans(stats[["ne"]], na.rm=T)
ne <- c(ne_h_means, mean(stats$ne))
...
sem_ne <- apply(stats$ne, 2, function(x) sd(x) / sqrt(length(x))) # 2 means work along columns
sem_ne_mean <- sd(stats$ne) / sqrt(length(stats$ne))

sem_ne <- c(sem_ne, sem_ne_mean)
sem_ne <- as.data.frame(sem_ne)
rownames(sem_ne) <- Pops
colnames(sem_ne) <- "SE_ne"
