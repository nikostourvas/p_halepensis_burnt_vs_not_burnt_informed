...
summ_p <- list()
for(i in 1: length(obj_list)){
  summ_p[[i]] <- locus_table(obj_list[[i]])
}

  # isolate uhe mean value for each pop
uhe <- vector(mode = "double")
for(i in 1: length(obj_list)){
uhe[i] <- summ_p[[i]][ length(summ_p[[i]][,3]) , 3 ] # I pick the last line
}
uhe_col <- c(uhe, mean(uhe)) # column to be added to the summary table

  # isolate uhe loci values for each pop
summ_uhe <- list()
for(i in 1: length(obj_list)){
summ_uhe[[i]] <- summ_p[[i]][ -length(summ_p[[i]][,3]) , 3 ] # I pick all lines but the last
}
...
sem_uhe_2 <- sd(unlist(summ_uhe)) / sqrt(length(unlist(summ_uhe)))

    #combine standard errors
sem_uhe_col <- c(sem_uhe, sem_uhe_2)

  # isolate na mean value for each pop
na <- vector(mode = "double")
for(i in 1: length(obj_list)){
na[i] <- summ_p[[i]][ length(summ_p[[i]][,1]) , 1 ] # I pick the last line
}
...
summ_na <- list()
for(i in 1: length(obj_list)){
summ_na[[i]] <- summ_p[[i]][ -length(summ_p[[i]][,1]) , 1 ] # I pick all lines but the last
}
  #calculate standard error of the mean
sem_na <- list()
for(i in 1: length(obj_list)){
...
# ne <- colMeans(stats[["ne"]], na.rm=T)

#ne adegenet
summ_list <- list()
for(i in 1: length(obj_list)){
  summ_list[[i]] <- summary(obj_list[[i]])
}

ne <- list()
for(i in 1:length(summ_list)){
  ne[[i]] <- 1 / (1 - summ_list[[i]][["Hexp"]])
}
ne <- as.data.frame(ne)
ne <- colMeans(ne)
...
```{r dapc, eval=FALSE, fig.width=10, cache=T, dpi=300, include=FALSE}
set.seed(1994)
